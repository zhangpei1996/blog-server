...
export const avatarHandler = avatarUpload.single('avatar');
...
export const pictureHandler = pictureUpload.array('picture', 9);

// 将用户上传的原图，处理为几种不同的像素，以便不同场景使用
// 例如：在动态列表中显示小图，动态详情中显示中等图片，当用户点击图片则显示大图
export async function pictureResize(ctx: Context, next: Next) {
  // 获取所有的图像信息
  const { files } = ctx;
